...
server.tool(
  "mod_move_to",
  "Move toward target X/Z coordinates. PREFERRED METHOD for navigation — use this instead of manual look_facing + mod_move_player duration estimation. Automatically calculates direction, estimates travel time, detects 1-block obstacles ahead and auto-jumps. Water-aware: detects if player is in water and switches to swim mode (forward+jump, ~2 blocks/sec). Returns distance, durationMs, and mode ('walk' or 'swim'). After movement completes, ALWAYS call get_client_state to verify final position (movement is frame-rate dependent, may overshoot/undershoot by 1-2 blocks). Coordinate reference: x increases=east, x decreases=west; z increases=south, z decreases=north.",
  {
